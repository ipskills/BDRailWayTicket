...
class RailwayAPI:
    ...
    def handshake(self) -> dict:
        try:
            ...
            if resp.status_code == 200:
                ...
                return {"success": True, "cities": self.cities}
    ...
    def set_token(self, token: str) -> dict:
        ...
        try:
            ...
            if resp.status_code == 200 and data.get("status") == "OK":
                ...
                name = profile.get("name", profile.get("mobile", "User"))
                return {"success": True, "message": f"Token valid - Welcome {name}"}
            self.auth_token = None
    ...
    def get_all_trains(self) -> dict:
        try:
            ...
            return {"success": False, "message": "Failed to load trains"}
        except Exception as e:
            return {"success": False, "message": str(e)}

    def search_trips(self, from_city: str, to_city: str, date: str, seat_type: int = 1) -> dict:
        if not self.auth_token:
            return {"success": False, "message": "Login required. Paste your auth token."}
        try:
